...
const TestimonialCard = ({ rating, name, role, quote, image, isCentered = false }) => {
  ...
  return (
    <div className="backdrop-blur-[15px] bg-[rgba(255,255,255,0.05)] relative rounded-[7px] w-full max-w-[410px] p-[25px] md:p-[30px] border border-[rgba(255,255,255,0.05)] flex flex-col justify-between min-h-[350px]">
      ...
      <PlusIcon size={16} className="absolute start-[15px] top-[15px] md:start-[324px] md:top-[30px] opacity-40 md:size-[20px]" />
      <PlusIcon size={16} className="absolute end-[15px] bottom-[15px] md:end-[20px] md:top-[-350px] opacity-40 md:size-[20px]" />
    </div>
  );
};
...
export const TestimonialSection = () => {
  const { language } = useLanguage();
  const t = langData[language.toLowerCase()].testimonial_section;

  return (
    <ScrollFadeIn>
      <section className="w-full flex justify-center py-10 md:py-16 px-6 md:px-8 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 w-full place-items-center">
            {t.testimonials.map((item, i) => (
              <TestimonialCard
                ...
                role={item.role}
                quote={item.quote}
                image={i === 0 ? imgImage : i === 1 ? imgImage1 : imgImage2}
              />
            ))}
          </div>
        </div>
