  ...
  return (
    <nav className="fixed top-4 md:top-5 left-1/2 -translate-x-1/2 w-[calc(100%-32px)] md:w-[calc(100%-64px)] max-w-[1260px] z-50">
      <div className="backdrop-blur-[25px] bg-black/40 md:bg-white/5  rounded-2xl md:rounded-full px-6 md:px-4 py-3 md:py-3 flex items-center justify-between relative isolate">
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 p-6 bg-black/90 backdrop-blur-xl border border-white/10 rounded-2xl lg:hidden flex flex-col gap-6 items-start z-[60]">
            <Link to="/" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>Home</Link>
            <Link to="/about" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/about') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>About</Link>
            <Link to="/services" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/services') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>Services</Link>
            <Link to="/industries" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/industries') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>Industries</Link>
            <Link to="/products" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/products') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>Products</Link>
            <Link to="/insights" className={`font-['Geist'] font-semibold text-lg w-full py-2 border-b border-white/5 transition-colors ${isActive('/insights') ? 'text-white' : 'text-white/75'}`} onClick={() => setIsOpen(false)}>Insights</Link>
            <Link
              to="/contact"
              className="sm:hidden w-full bg-gradient-to-tr from-[#001ca9] to-[#046ce4] py-4 rounded-xl text-white font-['Geist'] font-medium text-sm tracking-[0.5px] uppercase text-center"
              onClick={() => setIsOpen(false)}
